...
int main()
{
    ...
    // Define um número mínimo de arestas para garantir que o grafo seja denso
    int minEdges = N - 1 + (N * (N - 1) / 4); // Ajuste a fração conforme necessário
    int M = minEdges + rand() % (N * (N - 1) / 2 - minEdges + 1); // Gera o número de arestas entre minEdges e N*(N-1)/2

    vector<pair<int, int>> edges;
    generateSpanningTree(N, edges);

    // Adiciona novas arestas ao grafo até completar M
    while (edges.size() < M)
    {
        int u = 1 + rand() % N;
        int v = 1 + rand() % N;
